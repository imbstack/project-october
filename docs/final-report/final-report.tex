...
The backend recommender service is a Scala~\cite{scala} application that uses
MongoDB~\cite{mongodb} as its persistent data store. The frontend communicates
with it using the API that we defined using thrift.  When a post is submitted --
either through a user submission or via a feed -- the results are sent over.
When a user loads the homepage or searches for a string, the backend is invoked
...
\begin{thebibliography}{6}
  ...
  \bibitem{scala} \url{http://www.scala-lang.org/}
